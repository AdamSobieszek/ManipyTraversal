...
class Trainer(object):
    ...
    def train(self, generator, support_sets, reconstructor):
        ...
        # Set `generator` to evaluation mode, `support_sets` and `reconstructor` to training mode, and move to device
        ...
        for iteration in range(starting_iter, self.params.max_iter + 1):
            ...
            # Generate images for the shifted latent codes
            index = torch.randint(0, self.params.num_support_sets, (1,), device=self.device)
            half_range = self.params.num_support_timesteps // 2
            # Sample integer timestep index on device, then cast to z dtype (no grad needed)
            ...
            # Build target vector on correct device; use repeat (not expand) for MPS compatibility
            ...
            preds = torch.argmax(predicted_support_sets_indices, dim=1)
