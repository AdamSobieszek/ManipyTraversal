...
class PDEState:
    ...
    def __init__(self,
                 f: nn.Module,
                 psi: nn.Module,
                 x_projection: nn.Module,
                 z: torch.Tensor,
                 direction: int = +1,
                 **config):
        self.f_m = f
        self.psi_m = psi
        self.x_projection_m = x_projection
    ...
    def x_projection(self) -> torch.Tensor:
        return self.x_projection_m(self.x())

    def f(self) -> torch.Tensor:
        if "f" not in self.state:
            self.state["f"] = self.f_m(self.x_projection())
        return self.state["f"]

    def f_grad(self) -> torch.Tensor:
        if "f_grad" not in self.state:
            create_graph = bool(self.cfg["track_param_through_xgrads"])
            self.state["f_grad"] = grad(self.f().sum(), self.x_projection(), create_graph=create_graph)[0]
    ...
    def psi(self, when: When = "now") -> torch.Tensor:
        key = ("psi", when)
        if key not in self.state:
            self.state[key] = self.psi_m(self.x_projection(), self._t(when))
        return self.state[key]
    ...
    def psi_grad(self, when: When = "now") -> torch.Tensor:
        ...
        if key not in self.state:
            create_graph = bool(self.cfg["track_param_through_xgrads"])
            self.state[key] = grad(self.psi(when).sum(), self.x_projection(), create_graph=create_graph)[0]
        return self.state[key]
    ...
    def psi_t(self, when: When = "now") -> torch.Tensor:
        ...
        x_fixed = self.x_projection().detach()
        t_in = self._t(when)
        if self.cfg["detach_time_for_psi"]:
            t_in = t_in.detach()
