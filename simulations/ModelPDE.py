...
from pde_losses import build_losses 
...
class ZeroConvolution(nn.Module):
    def __init__(self, *shape: int, full_like: float = 0.0):
        super().__init__()
        self.weight = nn.Parameter(torch.full(shape, float(full_like)))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if len(x.shape) != len(self.weight.shape):
            for _ in range(len(x.shape) - len(self.weight.shape)):
...
class StackedSemanticPotential(nn.Module):
    ...
    def __init__(
        ...
        # self.zero_conv1 = ZeroConvolution(1, self.K, self.n_hidden, full_like=1.0)

        self.fc2 = StackedLinear(self.K, self.n_hidden, self.n_hidden)
        self.act2 = activation
        # self.zero_conv2 = ZeroConvolution(1, self.K, self.n_hidden, full_like=1.0)

        self.fc3 = StackedLinear(self.K, self.n_hidden, self.n_hidden)
        self.act3 = activation

        self.fc4 = StackedLinear(self.K, self.n_hidden, self.n_out)
        # self.zero_conv4 = ZeroConvolution(1, self.K, self.n_out, full_like=1.0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Stacked MLP body
        h = self.fc1(x)
        h = self.act1(h) + h # + self.zero_conv1(h)
        h = self.fc2(h)
        h = self.act2(h) + h # + self.zero_conv2(h)
        h = self.fc3(h)
        h = self.act3(h) 
...
class ModelPDE(nn.Module):
    ...
    @torch.enable_grad()
    def inference(self, z: torch.Tensor, t_index: torch.Tensor = None, dt: torch.Tensor = 1.0, direction: int = +1, return_all: bool = False) -> List[torch.Tensor]:
        if len(z.shape) == 3:
            B, K, D = z.shape
            z_curr = z
        else:
        ...
        if return_all:
            ...
            return potential_preds, z_curr, x_next-z_curr
        else:
