...
class ImageViz:
    ...
    def plot_information_matrix_panel(
        *,
        ...
        ax11.plot(xs, yD_L, linewidth=1.8, label="d_eff(Λ)")
        ax11.plot(xs, yD_S, linewidth=1.8, label="d_eff(Σ)")
        ax11.set_title(r"$d_{\mathrm{eff}}(\Sigma)=(\mathrm{tr}\,\Sigma)^2/\mathrm{tr}(\Sigma^2)$")
        ax11.set_xlabel("opt step"); ax11.set_ylabel("d_eff(Σ)")
        ax11.grid(True, linewidth=0.3, alpha=0.4)
    ...
    def plot_final_points_nonlinear_embeddings(
        ...
        *,
        ...
        perplexity = float(min(15.0, ((N - 1) / 3.0)))
        ...
        Y_isomap = Isomap(n_neighbors=n_neighbors, n_components=2).fit_transform(X)
...
# --------------------------
@torch.no_grad()
def _dt_legacy_uniform(B: int, device, half_range: int, low: int = 4000, high: int = 5500, dtype=torch.float32):
